...
namespace LuaScriptingEngine
{
    ...
    public static class LuaBuildings
    {
        ...
        {
            Debug.Log(GetBuildingDef(prefab_id).BuildingComplete.GetComponent<LogicOperationalController>().unNetworkedValue);
        }
        ...
        static Dictionary<string, newbuilding> buildingtable = new Dictionary<string, newbuilding>();
        static public newbuilding MakeOrGetBuilding(string id)
        {
            if (buildingtable.ContainsKey(id)) return buildingtable[id];
            var nb = new newbuilding();
            nb.ID = id;
            buildingtable.Add(id, nb);
            return nb;
        }
